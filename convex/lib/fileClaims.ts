...
/**
 * Whether a stored file is somebody's licence: a file in someone's licence
 * wallet (`memberLicenceFiles`, `memberLicences.ts`).
 *
...
export async function heldAsLicence(
  ...
  const inWallet = await ctx.db
    .query('memberLicenceFiles')
    .withIndex('by_storage', (q) => q.eq('storageId', storageId))
    .first()
  return inWallet !== null
}
