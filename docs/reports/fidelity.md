...
| Red                   | `#ff0000` — band, section headings, the treatment table's header row | The app's `#DC2A1F` (red-fill) | This is PestM8's document now. Pure red is neither what the app uses on screen nor what a laser printer flatters; the `#fff2f2` label tint is kept exactly. *Amended 2026-09-26:* was the brand `#FF3B30`, which holds the band's and table header's white text at 3.5:1; red-fill holds it at 4.8:1 |
