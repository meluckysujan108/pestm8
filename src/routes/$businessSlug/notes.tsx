...
const searchSchema = z.object({
  ...
  // Absent means My notes: the section is first a personal notebook.
  filter: z
    .enum([
      'mine',
      'all',
...
export const Route = createFileRoute('/$businessSlug/notes')({
  ...
  loader: ({ context: { queryClient, business }, location }) => {
    const filter = (searchParam(location, 'filter') ?? 'mine') as LibraryFilter
...
function NotesPage() {
  ...
  return (
    <NotesLibrary
      ...
      onFilter={(next) =>
        navigate({
          search: (prev) => ({ ...prev, filter: next === 'mine' ? undefined : next, noteId: undefined }),
          replace: true,
        })
      }
      onOpen={(id) =>
