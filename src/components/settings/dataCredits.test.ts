...
/**
 * Settings → About is the only place the app names the licence of the suburb
 * tables it ships. It must say what the tables themselves say
 * (scripts/build-localities.mjs writes it into each header), not a licence
 * the data is not under.
 */
...
const about = words(
  readFileSync(
    resolve(root, 'src/routes/$businessSlug/settings/about.tsx'),
    'utf8',
  ),
)
const localities = resolve(root, 'src/lib/localities')

describe('the suburb data credit in Settings → About', () => {
