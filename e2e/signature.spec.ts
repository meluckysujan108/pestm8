...
test('signing commits once, and records what was signed', async ({ page }) => {
  ...
  const reportId = await createReport(owner.client, { businessId, propertyId }, 'termiteManagementCert')
  ...
  page.on('request', (request) => {
    if (request.method() === 'POST' && request.url().includes('storage/upload')) uploads++
  })

  await signInViaUi(page, email)
  await page.goto(sectionUrl(slug, reportId, 'termiteManagementCert', 'installerSignature'))
  ...
  await expect(sheet.getByText(/I hereby certify that the subterranean termite/)).toBeVisible()
  ...
  await expect(
    page.getByRole('button', { name: 'Installer Signature — signed, sign again' }),
  ).toBeVisible()
...
test('a nothing-drawn pad cannot be committed', async ({ page }) => {
  const { email, owner, businessId, slug, propertyId } = await setup('sig-blank')
  const reportId = await createReport(owner.client, { businessId, propertyId }, 'serviceReport')

  await signInViaUi(page, email)
  await page.goto(sectionUrl(slug, reportId, 'serviceReport', 'technicianSignature'))
  await builderReady(page)
...
test('a saved signature is offered to its owner, and to nobody else', async ({ page }) => {
  const { email, owner, businessId, slug, propertyId } = await setup('sig-saved')
  const first = await createReport(owner.client, { businessId, propertyId }, 'serviceReport')

  await signInViaUi(page, email)
  await page.goto(sectionUrl(slug, first, 'serviceReport', 'technicianSignature'))
  await builderReady(page)
  ...
  const second = await createReport(owner.client, { businessId, propertyId }, 'serviceReport')
  await page.goto(sectionUrl(slug, second, 'serviceReport', 'technicianSignature'))
  await builderReady(page)
  await page.getByRole('button', { name: "Technician's Signature — sign" }).click()
  ...
  const report = await owner.client.query(api.reports.get, { businessId, reportId: second })
  ...
  await builderReady(page)
  await page.getByRole('button', { name: 'Signature — sign', exact: true }).click()
  await expect(page.getByRole('button', { name: 'Use my saved signature' })).toHaveCount(0)
})

test('a client signing sees the statement and gives their name', async ({ page }) => {
  const { email, owner, businessId, slug, propertyId } = await setup('sig-client')
  const reportId = await createReport(owner.client, { businessId, propertyId }, 'timberPestInspection')

  await signInViaUi(page, email)
  await page.goto(sectionUrl(slug, reportId, 'timberPestInspection', 'clientSignature'))
  await builderReady(page)
  ...
  const sheet = page.getByRole('dialog')
  await expect(sheet.getByText(/The Client acknowledges and agrees with the contents/)).toBeVisible()
